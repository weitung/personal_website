...
class WeitungPersonalPageStyle(Famcy.FamcyStyle):
	def __init__(self, title=""):
		super(WeitungPersonalPageStyle, self).__init__(sijax_enable=True)
	...
	def render(self, extra_script, content, background_flag=False, route="", time=5000, form_init_js=None, end_script="", **kwargs):
		...
		if kwargs["event_source_flag"]:
			...
			_event_source_script = ''
		return render_template("portfolio.html", event_source=_event_source_script, font_theme=font_theme, form_init_js=form_init_js, load_spinner=load_spinner, color_theme=color_theme, html_header=html_header, content=content, extra_script=extra_script, end_js=end_js, body_on_load=body_on_load)
